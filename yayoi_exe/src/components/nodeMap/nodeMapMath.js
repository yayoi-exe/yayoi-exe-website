export const ACCENT_RGB = { r: 245, g: 183, b: 0 };
export const LINK_DISTANCE = 180;
export const MAX_SPEED = 0.02;
// 接続が2本以下の点は作らない（各ノード最低3本）
export const MIN_DEGREE = 3;
export const SEED_STORAGE_KEY = 'yayoi-node-map-seed';

export function nodeCountForArea(width, height) {
    const area = width * height;
    return Math.max(12, Math.min(28, Math.round(area / 42000)));
}

/** 訪問者ごとに固定されるベースシードを取得（初回のみランダム生成） */
export function getVisitorSeed() {
    try {
...
export function getDailySeed(visitorSeed, date = new Date()) {
...
export function createNodes(count, random) {
...
export function buildEdges(projected) {
...
export function drawFrame(ctx, width, height, nodes, animate) {
